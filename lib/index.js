...
exports.Config = koishi_1.Schema.object({
    ...
    hideLockAndProtection: koishi_1.Schema.boolean().default(false).description('隐藏锁定模式和保护模式功能，开启后相关指令将不可用，状态信息也不会显示'),
});
/**
 * 票券ID到中文名称的映射
 */
...
function checkAllStatusFalse(result) {
    return (result.LoginStatus === false &&
 ...
 * 支持从文本消息或图片消息中提取
 */
async function extractQRCodeFromSession(session, ctx) {
    // 1. 检查文本消息中是否包含SGID
...
async function promptForRebind(session, ctx, api, binding, timeout = 60000) {
    ...
    let promptMessageId;
    try {
        const sentMessage = await session.send(`❌ 二维码对应ID无法登陆，您需要重新绑定新的二维码，请在${actualTimeout / 1000}秒内直接发送SGID或二维码照片`);
        // 尝试从返回的消息中提取消息ID
    ...
    return new Promise((resolve) => {
        ...
        const messageHandler = async (nextSession) => {
            if (resolved)
                return;
            // 只处理来自同一用户的消息
            ...
            if (session.channelId) {
                if (nextSession.channelId !== session.channelId)
                    return;
            }
            else {
                // 私聊消息：不能是群组消息
                if (nextSession.channelId)
                    return;
            }
            // 忽略命令消息（避免响应其他命令）
            ...
            try {
                ...
                if (content && content.startsWith('SGWCMAID')) {
                    ...
                    if (qrCode && qrCode.startsWith('SGWCMAID')) {
                        ...
                        // 图片无法解析，提示用户但继续等待（不立即失败）
                        logger.debug('图片中无法提取二维码，继续等待用户发送SGID文本');
                        await session.send('⚠️ 无法从图片中识别二维码，请直接发送SGID文本（SGWCMAID开头）').catch(() => { });
                        return; // 继续等待，不resolve也不reject
                    }
                }
                if (!qrCode || !qrCode.startsWith('SGWCMAID')) {
                    // 如果有内容但不是SGID，提示用户继续等待
                    if (content) {
                        await session.send('⚠️ 未识别到有效的二维码格式，请发送SGID文本（SGWCMAID开头）').catch(() => { });
                        return; // 继续等待
                    }
                    return; // 空消息，继续等待
                }
  ...
  - 使用 /mai状态 --expired 可查看过期票券`;
    ...
    ctx.command('mai绑定 [qrCode:text]', '绑定舞萌DX账号')
        ...
        try {
            ...
            if (!qrCode) {
                ...
                let promptMessageId;
                try {
                    const sentMessage = await session.send(`请在${actualTimeout / 1000}秒内直接发送SGID或二维码照片`);
                    if (typeof sentMessage === 'string') {
                        promptMessageId = sentMessage;
                    }
                    else if (sentMessage && sentMessage.messageId) {
                ...
                try {
                    ...
                    // 使用Promise同时等待session.prompt和message事件
                    // 不立即resolve，只有当真正收到有效二维码或超时才resolve
                    let resolved = false;
                    ...
                    qrCode = await new Promise((resolve, reject) => {
                        ...
                        // 方法1: 监听message事件（支持文本和图片）
                        messageHandler = async (nextSession) => {
                            if (resolved)
                                return;
                            logger.debug(`收到message事件: userId=${nextSession.userId}, channelId=${nextSession.channelId}`);
                            // 检查用户和频道
                            if (nextSession.userId !== session.userId)
                                return;
                            if (session.channelId && nextSession.channelId !== session.channelId)
                                return;
                            if (!session.channelId && nextSession.channelId)
                                return;
                            const content = nextSession.content?.trim() || '';
                            const hasImage = nextSession.elements?.some(el => el.type === 'image' || el.type === 'img');
                            // 忽略命令
                            ...
                            if (hasImage) {
                                ...
                                try {
                                    ...
                                    await session.send('⚠️ 处理图片时发生错误，请直接发送SGID文本（SGWCMAID开头）').catch(() => { });
                                    return; // 继续等待
                                }
                            }
                        };
                        ...
                        // 方法2: 同时使用session.prompt（作为辅助，处理文本消息）
                        session.prompt(actualTimeout).then((text) => {
                            if (resolved)
                                return;
                            if (!text)
                                return;
                            ...
                            if (trimmed.startsWith('SGWCMAID')) {
                                ...
                                // session.prompt收到了非二维码文本，继续等待（不resolve）
                                logger.debug(`session.prompt收到非二维码消息: ${trimmed.substring(0, 30)}`);
                            }
                        }).catch((error) => {
                            // session.prompt失败不影响，继续等待message事件
            ...
            if (result.LoginStatus === false &&
                ...
                if (rebindResult.success && rebindResult.newBinding) {
                    ...
                    try {
                        ...
                        const retryResult = await api.uploadScore(rebindResult.newBinding.maiUid, machineInfo.clientId, machineInfo.regionId, machineInfo.placeId, machineInfo.placeName, machineInfo.regionName, scoreData.musicId, scoreData.level, scoreData.achievement, scoreData.fcStatus, scoreData.syncStatus, scoreData.dxScore);
                        if (retryResult.LoginStatus === false &&
    ...
    ctx.command('mai管理员关闭所有锁定和保护', '管理员一键关闭所有人的锁定模式和保护模式（需要auth等级3以上）')
        ...
        try {
            // 确认操作（如果未使用 -bypass）
            if (!options?.bypass) {
                const confirm = await promptYesLocal(session, '⚠️ 即将关闭所有用户的锁定模式和保护模式\n此操作将影响所有已绑定账号的用户\n确认继续？');
                if (!confirm) {
                    return '操作已取消';
                }
            }
