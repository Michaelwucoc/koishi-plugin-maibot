...
function apply(ctx, config) {
    ...
    ctx.command('mai逃离小黑屋', '登出MaiDX以逃离小黑屋')
        ...
        try {
            ...
            const binding = bindings[0];
            const result = await api.logout(binding.maiUid, DEFAULT_MACHINE_INFO.regionId.toString(), DEFAULT_MACHINE_INFO.clientId, DEFAULT_MACHINE_INFO.placeId.toString(), DEFAULT_TURNSTILE_TOKEN);
    ...
    ctx.command('mai发票 [multiple:number]', '为账号发放功能票（2-6倍）')
        ...
        try {
            ...
            const ticketResult = await api.getTicket(binding.maiUid, multiple, DEFAULT_MACHINE_INFO.clientId, DEFAULT_MACHINE_INFO.regionId, DEFAULT_MACHINE_INFO.placeId, DEFAULT_MACHINE_INFO.placeName, DEFAULT_MACHINE_INFO.regionName);
    ...
    ctx.command('mai清票', '清空账号的所有功能票')
        ...
        try {
            ...
            const result = await api.clearTicket(binding.maiUid, DEFAULT_MACHINE_INFO.clientId, DEFAULT_MACHINE_INFO.regionId, DEFAULT_MACHINE_INFO.placeId, DEFAULT_MACHINE_INFO.placeName, DEFAULT_MACHINE_INFO.regionName);
