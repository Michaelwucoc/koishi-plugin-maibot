    ...
    const scheduleLxB50Notification = (session, taskId) => {
        ...
        const poll = async () => {
            ...
            try {
                ...
                await bot.sendMessage(channelId, `${mention} 落雪B50任务 ${taskId} 在预设时间内仍未完成，请稍后使用 /mai查询落雪B50 手动确认`, guildId);
            }
            catch (error) {
                ...
                await bot.sendMessage(channelId, `${mention} 落雪B50任务 ${taskId} 状态查询多次失败，请使用 /mai查询落雪B50 手动确认`, guildId);
            }
        };
