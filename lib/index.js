...
async function promptForRebind(session, ctx, api, binding, timeout = 60000) {
    ...
    let promptMessageId;
    try {
        const sentMessage = await session.send(`❌ 二维码对应ID无法登陆，您需要重新绑定新的二维码，请在${actualTimeout / 1000}秒内直接发送SGID（长按玩家二维码识别后发送）`);
        // 尝试从返回的消息中提取消息ID
    ...
    ctx.command('mai绑定 [qrCode:text]', '绑定舞萌DX账号')
        ...
        try {
            ...
            if (!qrCode) {
                ...
                let promptMessageId;
                try {
                    const sentMessage = await session.send(`请在${actualTimeout / 1000}秒内直接发送SGID（长按玩家二维码识别后发送）`);
                    if (typeof sentMessage === 'string') {
                        promptMessageId = sentMessage;
                    }
                    else if (sentMessage && sentMessage.messageId) {
